...
| **SELECT＋FLOW＋SOUL pooled**：N=30,787；主要 kidney/CV-death composite 973 vs 1,134，HR 0.84（0.77–0.91）；較窄、排除 CV death 的次要複合 347 vs 416，HR 0.80（0.69–0.92） | `SELECT-FLOW-SOUL-POOLED-2026` | Structured abstract Methods/Findings；PMID 42567173；電子出版 2026-08-07 | Pooled participant-level aggregate analysis；摘要已核實兩個終點定義；ahead of print at cutoff | 三試驗 Venn＋兩個 pooled aggregate；可畫 endpoint definition bars，**不與母試驗作 HR 排名** | Pooled estimate 包含 FLOW／SELECT／SOUL 資料，與母試驗統計相依；不得當成三個獨立估計比較。族群、劑量與 route 亦異質；不是新的隨機 cohort。不要用原文尚未取得的圖。 |
