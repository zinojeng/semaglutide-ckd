...
| **無 semaglutide-specific acute dip**：baseline→week 12 絕對變化 −1.07 vs −1.05；差 −0.03（−0.56–0.51）；chronic week 12→end slope −2.36 vs −3.30，差 +0.94（0.62–1.26） | `FLOW-PRIMARY-2024` | Fig.1D／Results，期刊 pp.114–115／PDF pp.6–7；Table 2 p.116／PDF p.8 | 預先指定支持性 slope 分解 | 放大前 12 週 inset，再接 chronic slope | acute row 是「12 週絕對變化」，不是每年 slope；不可誤標 `/year`。 |
| **week 104 cystatin-C eGFR 差** +3.39（2.63–4.15）；creatinine counterpart +3.30（2.43–4.17，post hoc） | `FLOW-PRIMARY-2024` | Table 2，期刊 p.116／PDF p.8；Results p.115／PDF p.7 | cystatin-C 為預先指定支持性；creatinine counterpart 事後 | 小型 concordance panel（creatinine vs cystatin C） | 不要把兩種 filtration markers 當成兩個獨立臨床終點。 |
...
| **SGLT2i subgroup cystatin-C concordance**：五成分 HR 0.74（0.47–1.16）vs 0.70（0.60–0.82），P-int=.844；slope difference +0.92（0.16–1.68）vs +1.55（1.21–1.88），P-int=.142 | `FLOW-SGLT2-2024` | Results／Table 1；local lines 411–450；Extended Data／Supplement locators in local file | 事後 cystatin-C analysis | appendix concordance table | 可作敏感度支持，不能取代 creatinine-based prespecified hard outcome 或證明 drug additivity。 |
...
| **MRA subgroup 3 年絕對效果**：RD −0.11（−0.20, −0.01）、NNT 9 users；RD −0.04（−0.07, −0.02）、NNT 23 non-users；四成分 HR 0.38（0.15–0.84）vs 0.82（0.68–0.99），P-int=.068；RRT P-int=.027、users 僅 11 events | `FLOW-MRA-2025` | Results／Figs.1–2／Supplementary Table 2；local lines 203–213 | 探索性 subgroup estimates | appendix absolute-effect bars；NNT 明標 exploratory | NNT 不是 trial-level；RRT 交互作用事件極少、未校正，不能作臨床排序。 |
...
| **CV phenotype**：ASCVD n=1,198，HR 0.80（0.63–1.02）vs complement 0.74（0.62–0.89），P-int=.62，3y NNT 22；HF n=678，0.67（0.49–0.93）vs 0.79（0.67–0.93），P-int=.40，NNT 13；high-risk/no established CVD n=1,329/2,000，0.73（0.58–0.91）vs 0.73（0.49–1.08），P-int=.99，NNT 17 | `FLOW-CVPHENOTYPE-2026` | Structured abstract Results；DOI 10.1016/j.jacc.2026.02.5125；PMID 42233552 | 預先指定 subgroup；摘要層級 | 三列 phenotype forest＋各自 NNT | NNT 只適用這三個 CV phenotype，不可套到 KDIGO／UACR strata；無 interaction 不是 equivalence。 |
...
| **SELECT＋FLOW＋SOUL pooled**：N=30,787；含 CV death composite 973 vs 1,134，HR 0.84（0.77–0.91）；kidney-specific 347 vs 416，HR 0.80（0.69–0.92） | `SELECT-FLOW-SOUL-POOLED-2026` | Structured abstract Findings；PMID 42567173；電子出版 2026-08-07 | Pooled individual-/trial-level analysis；摘要層級、ahead of print at cutoff | 三試驗 Venn／endpoint composition matrix＋兩個 pooled estimates | 族群、劑量、route、endpoint composition 異質；不是新的隨機 cohort。不要用原文尚未取得的圖。 |
...
- 「eGFR 全期與慢性斜率的差異見**圖 2**；前 12 週未見 semaglutide-specific acute dip（`FLOW-PRIMARY-2024`, Fig.1D/Table 2）。」
